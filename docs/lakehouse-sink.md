...
2. Build the connector in the `pulsar-io-lakehouse` directory.
   ...
   ```bash
   ls target
   pulsar-io-lakehouse-4.0.0.5.nar
   ```
   ...
   ```json
    {
        ...
        "archive": "connectors/pulsar-io-hudi-4.0.0.5.nar",
   ...
   ```json
    {
        ...
        "archive": "connectors/pulsar-io-hudi-4.0.0.5-cloud.nar",
        "parallelism": 1,
        "processingGuarantees": "EFFECTIVELY_ONCE",
        "configs":   {
            "type": "hudi",
...
- The Iceberg table that is stored in the file system
    ...
    ```json
    {
        ...
        "archive": "connectors/pulsar-io-lakehouse-4.0.0.5.nar",
        "processingGuarantees":"EFFECTIVELY_ONCE",
        "configs":{
            "type":"iceberg",
...
- The Iceberg table that is stored in cloud storage (AWS S3, GCS, or Azure)
    ...
    ```json
    {
        ...
        "archive": "connectors/pulsar-io-lakehouse-4.0.0.5-cloud.nar",
        "processingGuarantees":"EFFECTIVELY_ONCE",
        "configs":{
            "type":"iceberg",
            "maxCommitInterval":120,
...
1. Copy the NAR package to the Pulsar connectors directory.
    ...
    ```
    cp pulsar-io-lakehouse-4.0.0.5.nar PULSAR_HOME/connectors/pulsar-io-lakehouse-4.0.0.5.nar
    ```

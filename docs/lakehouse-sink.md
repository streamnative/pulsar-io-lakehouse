...
2. Build the connector in the `pulsar-io-lakehouse` directory.
   ...
   ```bash
   ls target
   pulsar-io-lakehouse-2.9.3.18.nar
   ```
   ...
   ```json
    {
        ...
        "archive": "connectors/pulsar-io-hudi-2.9.3.18.nar",
        "parallelism": 1,
        "configs":   {
            "type": "hudi",
...
- The Iceberg table that is stored in the file system
    ...
    ```json
    {
        ...
        "archive": "connectors/pulsar-io-lakehouse-2.9.3.18.nar",
...
1. Copy the NAR package to the Pulsar connectors directory.
    ...
    ```
    cp pulsar-io-lakehouse-2.9.3.18.nar PULSAR_HOME/connectors/pulsar-io-lakehouse-2.9.3.18.nar
    ```
